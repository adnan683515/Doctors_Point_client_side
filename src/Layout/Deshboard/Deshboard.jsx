...
import { FaUserMd, FaCalendarCheck, FaPills, FaUserInjured, FaUserPlus, FaPlusSquare } from "react-icons/fa";
...
const Deshboard = () => {
    ...
    const { data: allValue = {}, isLoading: ValueLoadng } = useQuery({
        queryKey: 'all-length',
        queryFn: async () => (await axiosurl.get('/allValuesCollectionsDeshboard')).data
    });

    const { data: recentDoctor = [], isLoading } = useQuery({
        queryKey: 'somedoctors',
        queryFn: async () => (await axiosurl.get('/adminForDoctorRecent')).data
    });

    const { data: someAppoinemnts = [], isLoading: appoinemtnLoading } = useQuery({
        queryKey: 'someapp',
        queryFn: async () => (await axiosurl.get('/someAppointments')).data
    });
    ...
    return (
        <div className="min-h-screen  bg-[#F2F9FF] flex flex-col sm:flex-col md:flex-row gap-4 p-2 md:p-6">


            <div className="w-full sm:w-full md:w-3/4 flex flex-col gap-4">
                ...
                <div className='w-full'>
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 700 }} aria-label="customized table">
                            ...
                            <TableBody>
                                {someAppoinemnts.map((row, index) => (
                                    <DisplayAppointmentDesh key={index} row={row} index={index} />
                                ))}
                            </TableBody>
                        </Table>
                    </TableContainer>
            ...
            <div className='w-full sm:w-full md:w-1/4 mt-6 md:mt-0 bg-white rounded-lg px-3 py-1 flex flex-col gap-4'>


                <div>
                    <div className='flex justify-between items-center mb-3'>
                        <h1 className='text-[12px] font-semibold'>Recent Doctor's</h1>
                        <Link className='flex bg-blue-100 rounded-full px-3 py-1 items-center gap-2'>
                    ...
                    {recentDoctor.length ? recentDoctor.map((item) => {
                        ...
                        return (
                            <div key={item?._id} className="flex items-center justify-between rounded-lg p-2 mb-2 bg-green-50">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 flex-shrink-0">
                                        <img className="w-full h-full object-cover rounded-full border border-gray-300" src={item?.image} alt={item?.name || "Doctor"} />
                                    </div>
                                    <div>
                                ...
                                <div className="text-right">
                                    <span className={`inline-block text-[8px] px-2 py-0.5 rounded-full font-medium mb-1 ${ckDay ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                                        {ckDay ? 'Available' : 'Not Available'}
                                    </span>
                                    <Link to={`/doctorDetails/${item?._id}`} className="flex items-center gap-1 cursor-pointer">
                ...
                <div>
                    <div className='flex justify-between items-center mb-3'>
                        <h1 className='text-[12px] font-semibold'>Recent Added Medicine</h1>
                        <Link className='flex bg-blue-100 rounded-full px-3 py-1 items-center gap-2'>
                            <span className='text-[10px]'>show more</span>
                        </Link>
                    </div>
